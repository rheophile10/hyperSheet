...
const flow = await page.evaluate(async () => {
  ...
  out.afterUnlock = state.cels.get("walletNote")?.v;
  await call("wallet.set", "anthropic", { type: "change", target: { value: "sk-ant-fake-e2e" } });
  out.afterSet = state.cels.get("walletNote")?.v;
  ...
  await call("wallet.unlock", null, { type: "change", target: { value: "hunter2" } });
  out.reUnlock = state.cels.get("walletNote")?.v;
  out.afterReUnlockResolves = resolveFn(state, "apiKey")("anthropic").resolve();
  // wrong password fails to decrypt
  await resolveFn(state, "wallet.lock")(state);
  await call("wallet.unlock", null, { type: "change", target: { value: "WRONG" } });
  out.wrongPw = state.cels.get("walletNote")?.v;
  return out;
});
console.log("  flow:", JSON.stringify(flow));
ok(/new wallet created/.test(String(flow.afterUnlock)), "unlock creates+persists the wallet (OPFS)");
...
ok(/unlock failed/.test(String(flow.wrongPw)), "wrong password fails to decrypt");
