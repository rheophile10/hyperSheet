# plastron-examples/keyboard

A musical keyboard demo — second consumer of plastron's
[`sound` segment](../../plastron/src/甲骨坑/sound.ts), the same
...
```bash
cd ~/projects/plastron/plastron-examples/keyboard
bun run dev
# → http://localhost:3001
```
...
- `import { createInitialState, resolveFn } from plastron` — boot a state.
...
- Sound segment source:
  `../../plastron/src/甲骨坑/sound.{ts,json}`
- Sound segment tests:
  `../../plastron/test/sound.test.mjs`
