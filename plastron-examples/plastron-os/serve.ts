...
// (a symlink at plastron-examples/doom/freedoom1.wad).
...
Bun.serve({
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (path === "/doom.wasm") {
      ...
      catch { return new Response("doom.wasm not built — see plastron-examples/doom/README.md", { status: 404 }); }
    }
    if (/^\/[\w.-]+\.wad$/i.test(path)) {
      try { return await respond(join(doomDir, path.slice(1))); }
      catch { return new Response(`${path.slice(1)} not present (symlink one into plastron-examples/doom/ — see README)`, { status: 404 }); }
    }
