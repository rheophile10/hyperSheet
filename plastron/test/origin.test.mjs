...
// (the README-rich-content test was removed — the readme is now an origin-user
//  document, with no .readme dom card / fx-code / try-it.)
...
test("=cels(3,3) makes a 3x3 worksheet of cels, each editable like 元", async () => {
  ...
  // SPARSE: no blank cels are minted — the range lives in ONE dims cel and an
  // address is keyed into state only when a value/formula is committed.
  assert.deepEqual(state.cels.get("g3x3.dims")?.v, { rows: 3, cols: 3 }, "g3x3.dims created (sparse range)");
  for (const a of ["g3x3.A1", "g3x3.C3", "g3x3.B2"]) assert.equal(state.cels.get(a), undefined, `${a} not keyed until committed`);
  assert.equal(new Set(cells(root).filter((c) => String(c.attrs["data-key"]).startsWith("g3x3.")).map((c) => c.attrs["data-key"])).size, 9, "9 g3x3 grid cells STILL render (addresses synthesized from dims)");
...
test("a syntax error surfaces under the editor instead of doing nothing", async () => {
  ...
  assert.deepEqual(state.cels.get("in.dims")?.v, { rows: 4, cols: 3 }, "the corrected formula ran (sparse: dims cel)");
});
...
test("introspection: inspect / segments / help-text return readable values", async () => {
  const { state, root, m } = await boot();
  ...
  await put(state, root, m, '=help("origin", "text")');
  const v = String(state.cels.get("元").v);
  assert.match(v, /functions/, "help text mode lists functions");
...
test("grid default name is g<r>x<c> — nested + different-shape grids don't collide", async () => {
  ...
  assert.ok(state.cels.get("g5x5.dims"), "first sheet g5x5 (sparse: dims cel)");
  await put(state, root, m, "=cels(4, 5)", "g5x5.A1"); // nested grid in a grid cell
  assert.ok(state.cels.get("g4x5.dims"), "nested grid g4x5 created — different shape, no collision");
...
test("grid(name,r,c,…) makes a workbook of named grids in one formula", async () => {
  ...
  assert.ok(state.cels.get("budget.dims") && state.cels.get("actuals.dims"), "both sheets created (sparse: dims cels)");
  await put(state, root, m, "");
  assert.equal(state.cels.get("budget.dims"), undefined, "deleting the formula sweeps all sheets");
});
