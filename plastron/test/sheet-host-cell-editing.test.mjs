...
const boot = async () => {
  ...
  // the desktop no longer auto-opens worksheets; open the README bundle (which
  // materializes the turtle_data + turtle_charts worksheets) as this suite's fixture —
  // the same content the 📖 Readme launcher opens.
...
test("formula bar: every worksheet window has a resizable formula bar (resize:vertical)", async () => {
  ...
  const bar = fxbarOf(root, "turtle_data");
  assert.ok(bar, "the turtle_data window has a formula bar");
  const input = fxInput(root, "turtle_data");
...
test("selection: single-click SELECTS a cell and the cell KEEPS rendering its value", async () => {
  ...
  assert.equal(cellVal(root, "turtle_data.B4"), "75", "B4 shows its value before selection");
  // single-clicking the cell value SELECTS (origin.select) — it must NOT open the editor
  const td = cellByKey(root, "turtle_data.B4");
  const value = byClass(td, "cell-value")[0];
  value.fire("click");
  await new Promise((r) => setTimeout(r, 0)); m.run();
  assert.equal(state.cels.get("元.selected")?.v, "turtle_data.B4", "the cell is now selected");
  assert.equal(state.cels.get("元.editing")?.v, null, "selection does NOT enter inline editing");
  assert.ok(!hasEditor(root, "turtle_data.B4"), "no editor swapped in — the value stays");
  assert.equal(cellVal(root, "turtle_data.B4"), "75", "the cell STILL renders its value after selection");
  // double-clicking opens the inline editor
  const td2 = cellByKey(root, "turtle_data.B4");
  const value2 = byClass(td2, "cell-value")[0];
  value2.fire("dblclick");
  await new Promise((r) => setTimeout(r, 0)); m.run();
  assert.equal(state.cels.get("元.editing")?.v, "turtle_data.B4", "double-click opened the inline editor");
});
...
test("formula bar: shows the SELECTED cell's source and is bound to 元.draft", async () => {
  ...
  assert.equal(String(fxInput(root, "turtle_data").attrs?.value ?? ""), "", "empty bar before selection");
  // select a turtle_data cell → its source appears in the turtle_data window's formula bar
...
test("formula bar: Enter in the bar commits the draft to the SELECTED cell (origin.key)", async () => {
  ...
  await new Promise((r) => setTimeout(r, 0)); m.run();
  assert.equal(state.cels.get("turtle_data.B4")?.v, 4242, "Enter in the bar committed the selected cell");
  assert.equal(state.cels.get("元.selected")?.v, "turtle_data.B4", "selection stays on the committed cell");
});
...
test("double-click: opens the inline editor for the cell", async () => {
  const { state, root, m } = await boot();
  assert.ok(!hasEditor(root, "turtle_data.B4"), "not editing yet");
  ...
  assert.ok(hasEditor(root, "turtle_data.B4"), "the inline editor opened");
});
...
test("fire: the bar's lightning-bolt (SVG) button dispatches origin.fire; bar order is [W wiki][bolt][textarea]", async () => {
  ...
  try {
    fire.fire("click");
    await new Promise((r) => setTimeout(r, 0));
    assert.equal(fired, "turtle_data.B4", "the fire button dispatches origin.fire with the selected cell key");
...
test("fire: origin.fire re-evaluates a formula cell", async () => {
  ...
  assert.equal(state.cels.get("turtle_data.B4")?.v, 2, "formula committed → 2");
  await resolveFn(state, "origin.select")(state, "turtle_data.B4"); m.run();
  await resolveFn(state, "origin.fire")(state); m.run();
  assert.equal(state.cels.get("turtle_data.B4")?.v, 2, "fire re-evaluated the formula (still 2)");
  assert.ok((state.cels.get("turtle_data.B4")).f, "the cell stays a FormulaCel after firing");
});
...
test("object/array value: renders as pretty-printed JSON in a <pre class=cell-pre>, NOT \"[object Object]\"", async () => {
  ...
  await resolveFn(state, "setValue")(state, "turtle_data.B4", msgs);
  await resolveFn(state, "runCycle")(state);
...
test("glyphs: cells render clean — NO per-cell ✎/✓/? glyphs", async () => {
  const { state, root, m } = await boot();
  // a plain value cell
  let td = cellByKey(root, "turtle_data.B4");
  ...
  await resolveFn(state, "setValue")(state, "元.draft", "=1+1");
  await resolveFn(state, "origin.run")(state, "turtle_data.B4"); m.run();
  td = cellByKey(root, "turtle_data.B4");
...
test("cell value: selection does NOT inject text into .cell-value", async () => {
  const { state, root, m } = await boot();
  await resolveFn(state, "origin.select")(state, "turtle_data.B4"); m.run();
  assert.equal(cellVal(root, "turtle_data.B4"), "75", ".cell-value still holds only the value (no glyph text)");
});
...
test("wiki: the formula bar carries a W wiki affordance dispatching wiki.open for the selected cell", async () => {
  ...
  try {
    ...
    wiki.fire("click");
    await new Promise((r) => setTimeout(r, 0));
    assert.equal(opened, "turtle_data.B4", "the bar's wiki affordance dispatches wiki.open for the selected cell");
  } finally { cel._fn = orig; }
});
...
test("tab reorder: dragging a tab over a sibling rewrites win.geom order; the strip renders by it", async () => {
  const { state, m } = await boot();
  // tab a fresh blank sheet into turtle_data so the strip is [turtle_data | tab1]
  await resolveFn(state, "winsheet.newtab")(state, "turtle_data"); m.run();
  const sibs = () => Object.keys(state.cels.get("win.geom").v).filter((k) => k === "turtle_data" || state.cels.get("win.geom").v[k]?.host === "turtle_data");
  const tabs = sibs();
  assert.ok(tabs.length >= 2, "turtle_data now hosts >=2 tabs");
  // stub elementsFromPoint so tabAtPoint(...) reports we're hovering turtle_data' chip
  const hovered = "turtle_data";
  globalThis.document.elementsFromPoint = () => [{ closest: (s) => (s === ".pl-tab" ? { getAttribute: () => hovered } : null) }];
  const dragged = tabs.find((t) => t !== "turtle_data"); // drag a non-turtle_data tab onto turtle_data' slot
  await resolveFn(state, "winsheet.tabGrab")(state, { host: "turtle_data", tab: dragged }, { clientX: 0, clientY: 0, pointerId: 1 });
  await resolveFn(state, "winsheet.tabMove")(state, { host: "turtle_data", tab: dragged }, { clientX: 10, clientY: 0 });
  const geom = state.cels.get("win.geom").v;
  assert.equal(typeof geom[dragged]?.order, "number", "the dragged tab got an explicit order");
  // dragged should now sort before turtle_data (it took turtle_data' slot)
  const ordered = sibs().sort((a, b) => (geom[a]?.order ?? 99) - (geom[b]?.order ?? 99));
  assert.ok(ordered.indexOf(dragged) < ordered.indexOf("turtle_data"), "the dragged tab moved ahead of turtle_data");
...
test("tab tear-off (drag): releasing a tab OFF its window clears its host → standalone window", async () => {
  const { state, m } = await boot();
  await resolveFn(state, "winsheet.newtab")(state, "turtle_data"); m.run();
  const tab = Object.keys(state.cels.get("win.geom").v).find((k) => state.cels.get("win.geom").v[k]?.host === "turtle_data");
  assert.ok(tab, "a tab hosted by turtle_data exists");
  // no elementsFromPoint on the mock document → drop lands in empty space → tear off
  await resolveFn(state, "winsheet.tabGrab")(state, { host: "turtle_data", tab }, { clientX: 0, clientY: 0, pointerId: 1 });
  await resolveFn(state, "winsheet.tabDrop")(state, { host: "turtle_data", tab }, { clientX: 900, clientY: 600 });
...
test("max/mid toggle: a non-maximized window shows ⛶ (maximize) and NOT ◱", async () => {
  const { root } = await boot();
  const glyphs = winBtns(root, "turtle_data").map(txt);
...
test("max/mid toggle: a maximized window shows ◱ (mid) and NOT ⛶", async () => {
  const { state, root, m } = await boot();
  await resolveFn(state, "winsheet.maximize")(state, "turtle_data"); m.run();
  assert.equal(state.cels.get("win.geom")?.v?.turtle_data?.max, 1, "win.geom[turtle_data].max set");
  const glyphs = winBtns(root, "turtle_data").map(txt);
  assert.ok(glyphs.includes("◱"), "shows ◱ mid when maximized");
  assert.ok(!glyphs.includes("⛶"), "does NOT also show ⛶");
  // mid clears it again → back to ⛶
  await resolveFn(state, "winsheet.mid")(state, "turtle_data"); m.run();
  assert.ok(!state.cels.get("win.geom")?.v?.turtle_data?.max, "mid cleared the max flag");
  const after = winBtns(root, "turtle_data").map(txt);
...
test("grab: offset is measured from the window's ACTUAL position (offsetLeft/Top), so the first move doesn't jump", async () => {
  ...
  await resolveFn(state, "winsheet.grab")(state, "turtle_data", event); m.run();
  ...
  await resolveFn(state, "winsheet.move")(state, "turtle_data", { clientX: 200, clientY: 250 }); m.run();
  const g = state.cels.get("win.geom")?.v?.turtle_data;
...
test("grab: with NO live rect (off-DOM) wsGrab falls back to win.geom default 60", async () => {
  ...
  await resolveFn(state, "winsheet.grab")(state, "turtle_data", { clientX: 100, clientY: 100, pointerId: 1 }); m.run();
  const d = state.cels.get("winsheet.drag")?.v;
  const g = state.cels.get("win.geom")?.v?.turtle_data ?? {};
...
test("dragged tab: while winsheet.tabdrag is set, the matching chip highlights blue (.drop-target)", async () => {
  const { state, root, m } = await boot();
  // tab a fresh sheet into turtle_data so there is a tab strip with >1 chip
  await resolveFn(state, "winsheet.newtab")(state, "turtle_data"); m.run();
  ...
  assert.ok(chipFor("turtle_data") && !hasClass(chipFor("turtle_data"), "drop-target"), "no highlight before a drag");
  // start dragging the tab1 tab → its chip (and ONLY its chip) lights up
  await resolveFn(state, "winsheet.tabGrab")(state, { host: "turtle_data", tab: "tab1" }, { clientX: 0, clientY: 0, pointerId: 1 }); m.run();
  assert.equal(state.cels.get("winsheet.tabdrag")?.v?.tab, "tab1", "winsheet.tabdrag records the dragged tab");
  const dragged = chipFor("tab1");
  ...
  const sibling = chipFor("turtle_data");
  ...
  globalThis.document.elementsFromPoint = () => [{ closest: (s) => (s === ".pl-window" ? { getAttribute: () => "turtle_data" } : null) }];
  try {
    await resolveFn(state, "winsheet.tabDrop")(state, { host: "turtle_data", tab: "tab1" }, { clientX: 0, clientY: 0 }); m.run();
