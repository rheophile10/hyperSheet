...
// ============================================================================
// The vault's secrets now live as cels in a SEALED `secrets` segment
...
// Bring the vault up, unlock it (sets module state + seal hooks), store a key.
const vaultWithKey = async () => {
  ...
  assert.equal(resolveFn(state, "locked")(), false, "vault unlocked");
...
test("per-segment consent (locked): apiKey is allowed for `clients` but refused for another segment", async () => {
  const state = await vaultWithKey();
