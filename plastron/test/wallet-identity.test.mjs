...
// ============================================================================
// The wallet's unlock mints (first time) or loads (subsequently) ONE identity
...
const unlock = (state, pw) =>
  call(state, "wallet.unlock", null, { type: "change", target: { value: pw } });
...
test("locked → identity() is the unlock placeholder, not a key", async () => {
  ...
  await resolveFn(state, "wallet.lock")(state); // module-global lock state — ensure locked
