...
import { isDefinitionCel, isFireable } from "../cels.js";
import { bumpDefGeneration } from "../卜/graph.js";
import { appendError, makeCelError } from "../cel-error.js";
import { getSegmentManifest, segmentEntries } from "../segments/cels.js";
 ...
 *  and live thereafter on their SegmentCel, so in-place is fine. */
 ...
 *  from an already-loaded SegmentCel. */
const lookupRole = (
  name: Key,
  state: State,
  batch: Map<Key, 冊>,
): Key | undefined => {
  return (batch.get(name) ?? getSegmentManifest(state, name))?.role;
};
...
export const validateManifests = (state: State, manifests: 冊[]): void => {
  ...
  const allKnown = new Map<Key, 冊>(segmentEntries(state));
  ...
  for (const m of manifests) {
    ...
    if (m.role === "user-space" && m.applications) {
      for (const dep of m.dependencies) {
        const depManifest = batch.get(dep) ?? getSegmentManifest(state, dep);
  ...
  if (warnings.length > 0) {
    // Best-effort warning surface; no formal logger in plastron yet.
...
export const inflateAllCels = (state: State, segments: 甲骨[]): void => {
  for (const seg of segments) {
    for (const dc of seg.cels) {
      ...
      // Replacing a DEFINITION cel (lambda/schema/compiler/channel/
      // range) invalidates everything compiled against the old one —
      // possibly in segments outside this batch. Stamp the generation;
      // out-of-batch consumers detect the mismatch at their next fire
      // and recompile (in-batch consumers compile fresh below anyway).
      if (existing && isDefinitionCel(cel)) bumpDefGeneration(state, cel);
      state.cels.set(dc.key, cel);
    }
  }
};
