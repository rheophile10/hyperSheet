...
// (or browser) now. See plastron/docs/2-roadmap/bun-upgrade.md.
...
const spawnWorker = async (state: State | undefined): Promise<PyWorkerHandle> => {
  ...
  // Sibling file in the compiled dist tree (tsc emits utils/py-worker.js
  // under py-compiler/). The URL is constructed at runtime so bundlers
  // (bun build, esbuild, vite) can rewrite it to the bundled worker
  // output path.
  const workerUrl = new URL("./utils/py-worker.js", import.meta.url);
