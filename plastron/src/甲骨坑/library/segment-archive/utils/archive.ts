...
import { resolveFn, getSegmentManifest, segmentEntries } from "../../../../kernel/index.js";
...
const roleOf = (state: State, name: Key): Role =>
  ((getSegmentManifest(state, name)?.role as Role | undefined) ?? "library");
...
const kernelClosure = (state: State): Set<Key> => {
  ...
  const queue: Key[] = [];
  for (const [name, m] of segmentEntries(state)) if (m.role === "kernel") queue.push(name);
  while (queue.length > 0) {
    const name = queue.shift()!;
    if (out.has(name)) continue;
    out.add(name);
    for (const dep of getSegmentManifest(state, name)?.dependencies ?? []) queue.push(dep);
  }
  return out;
};
...
const closureForRoles = (state: State, roots: Key[], includeRoles: Set<Role>): Set<Key> => {
  ...
  while (queue.length > 0) {
    ...
    seen.add(name);
    const m = getSegmentManifest(state, name);
    if (!m) continue;
...
export const buildArchive = async (state: State, includeNames: Set<Key>): Promise<Uint8Array> => {
  ...
  for (const seg of dumped.segments) {
    const m = manByName.get(seg.name) ?? getSegmentManifest(state, seg.name);
