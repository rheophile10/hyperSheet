// Browser-runtime harness. It runs only in the browser; these module-local ambient
// declarations let it type-check inside the kernel's runtime-agnostic tsconfig
// (no `dom` lib) WITHOUT leaking DOM globals into the rest of the build.
declare const window: { addEventListener(t: string, h: (e: KeyboardEvent) => void): void; removeEventListener(t: string, h: (e: KeyboardEvent) => void): void };
declare const document: { getElementById(id: string): HTMLCanvasElement | null; title: string };
declare function requestAnimationFrame(cb: (t: number) => void): number;
declare function cancelAnimationFrame(h: number): void;
interface WasmMemory { buffer: ArrayBuffer }
interface HTMLCanvasElement { width: number; height: number; getContext(id: "2d"): CanvasContext | null }
interface CanvasContext { putImageData(d: unknown, x: number, y: number): void; createImageData(w: number, h: number): { data: Uint8ClampedArray }; fillStyle: string; fillRect(x: number, y: number, w: number, h: number): void }
interface KeyboardEvent { code: string; key: string; keyCode: number; repeat: boolean; preventDefault(): void }
...
export function createDoomHarness(
  ...
  const mem = () => {
    const buf = (instance!.exports.memory as WasmMemory).buffer;
    return {
