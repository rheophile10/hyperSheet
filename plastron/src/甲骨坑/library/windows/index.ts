...
const windowFn: Fn = ((key: unknown, x: unknown, y: unknown, w: unknown, h: unknown, title: unknown, content: unknown): V => {
  const k = String(key ?? "win");
  const fx = num(x), fy = num(y), fw = num(w, 320), fh = num(h, 200);
  const body = isVnode(content) ? content : T(content == null ? "" : String(content));
  return el("div", { class: "pl-window", "data-win": k, style: `position:absolute;left:${fx}px;top:${fy}px;width:${fw}px;height:${fh}px;display:flex;flex-direction:column;border:1px solid #8886;border-radius:6px;background:Canvas;box-shadow:0 4px 16px #0004;overflow:hidden` }, [
