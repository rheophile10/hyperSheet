...
import { resolveFn, hasSegment } from "../../../kernel/index.js";
import webEditorSeed from "./甲骨.json" with { type: "json" };
...
// Save / Load), and a status line. The canonical default segment — those cels
// as pure data, with WEBEDIT_VIEW's template baked into webedit.view.f — lives
// in 甲骨.json; buildWebEditor loads and clones it, then applies opts.

type WebEditorSegment = 甲骨 & { version: string; role: "application"; dependencies: Key[] };
 ...
 *  applies opts: source/mount/path remap the matching ValueCels' `v` (source
 *  defaults to COUNTER_EXAMPLE — kept as an exported code constant since it's
 *  also a load-able preset); segment retags the manifest name and every cel's
 *  metadata.segment. Cel KEYS stay fixed ("webedit.source", …). */
export const buildWebEditor = (
  opts: BuildWebEditorOpts = {},
): WebEditorSegment => {
  const seg = structuredClone(webEditorSeed) as unknown as WebEditorSegment;
  ...
  // 甲骨.json seeds webedit.source as null (the giant preset isn't duplicated
  // into JSON); the builder fills the default from the code constant.
  setV("webedit.source", opts.source ?? COUNTER_EXAMPLE);
  if (opts.mount !== undefined) setV("webedit.mount", opts.mount);
  if (opts.path  !== undefined) setV("webedit.path",  opts.path);
...
const makeRun = (): Fn => async (state: State) => {
  const setFn = resolveFn(state, "setValue");
...
export const installWebEditorActions = async (
  ...
  const reg = (key: string, fn: Fn) =>
    setCelFn(state, key, { celType: "LockedLambdaCel", locked: true, fn, metadata: { segment, kind: "native" } });
