...
const apps = (state: State): AppEntry[] => (state.cels.get("os.apps")?.v as AppEntry[] | undefined) ?? [];
...
const launch: Fn = async (state: State, appId: Key, docName?: Key, opts?: LaunchOpts) => {
  ...
  await call(state, "setValueBatch", [["os.active", appId], ["os.doc", docName ?? null]], { flush: "all" });
  return state;
};

/** Switch to an already-loaded app. (dispatch-safe: ignores the event arg.) */
const switchTo: Fn = async (state: State, appId: Key) => {
  await call(state, "setValue", "os.active", appId, { flush: "all" });
  return state;
};
...
const exit: Fn = async (state: State, opts?: ExitOpts) => {
  const doc = state.cels.get("os.doc")?.v as Key | null | undefined;
  if (opts?.save && doc) await call(state, "saveUserSpace", doc);
  await call(state, "setValueBatch", [["os.active", "home"], ["os.doc", null]], { flush: "all" });
  return state;
};
...
const registerApp: Fn = async (state: State, app: AppEntry) => {
  ...
  await call(state, "setValue", "os.apps", [...current, app], { flush: "all" });
  return state;
};
