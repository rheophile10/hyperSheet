...
import {
  hydrate, dehydrate, flush, hydrateClosure,
  transitiveClosure, topoLevels, resolveFn,
  getSegmentManifest, hasSegment,
...
const isPrivateDep = (state: State, depName: Key, owner: 冊): boolean => {
  const m = getSegmentManifest(state, depName);
...
const privateClosure = (state: State, name: Key): Key[] => {
  const owner = getSegmentManifest(state, name);
...
const newUserSpace: Fn = async (
  ...
  const app = getSegmentManifest(state, applicationName);
...
const loadUserSpace: Fn = async (
  ...
  if (appName !== undefined && !hasSegment(state, appName)) {
...
const closeUserSpace: Fn = async (stateArg: unknown, nameArg: unknown): Promise<void> => {
  ...
  const rootFirst = topoLevels(
    closure,
    (n) => getSegmentManifest(state, n)?.dependencies ?? [],
    { memberSet },
  ).flat().reverse();
