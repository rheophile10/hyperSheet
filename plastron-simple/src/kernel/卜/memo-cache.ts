...
// access ordering. Closure factory (no classes in plastron-simple —
// see docs/1-design/3-accepted/00-ontology/de-class-kernel.md). See docs/1-design/3-accepted/03-caching/execution-hooks.md "L1 cache slot".
//
// Lookup:
...
export const makeMemoCache = (maxEntries = 128): MemoCache => {
  let root: Map<unknown, unknown> = new Map();
  let head: Entry | null = null;
  let tail: Entry | null = null;
  let size = 0;

  const linkAtHead = (entry: Entry): void => {
    entry.prev = null;
    entry.next = head;
  ...
  return {
    ...
    set(keys: readonly unknown[], value: unknown): void {
      ...
      size++;
      linkAtHead(entry);
      while (size > maxEntries) evictTail();
    },
